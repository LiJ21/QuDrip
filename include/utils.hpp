...
void fill_state(State<TIDX>& psi, const std::vector<value_type>& fill_vec,
                IDX&& index, idxv_type base_idx = 0, int tstp = 0) {
  TIDX& tot_index = psi.get_index();
  auto ncopies = tot_index.range() / index.range();
  auto counter = 0;
  for (tot_index[0]; tot_index < tot_index.range() && counter < ncopies;
       ++tot_index) {
    if (index == base_idx) {
...
template <class IDX1, class IDX2>
std::vector<double> state_to_p(int tstp, State<IDX1>& psi, IDX1& tot_index,
                               const IDX2& rho_index) {
...
Matrix herm_exp(const Matrix& A, value_type alpha) {
  ...
  expA.noalias() =
      eigensolver.eigenvectors() * temp * eigensolver.eigenvectors().adjoint();

  return expA;
}

template <typename Ir_R, typename Io_R, typename Ir_L, typename Io_L,
          typename State_r, typename State_o>
void reduce(int tstp, State_r& rho_r, const State_o& rho_o, Ir_R&& idx_rR,
            Io_R&& idx_oR, Ir_L&& idx_rL, Io_L&& idx_oL) {
...
void state_to_rho(L_IDX& l_idx, R_IDX& r_idx, State<RHO_IDX>& rho,
                  const State<R_IDX>& psi, int tstp = 0) {
...
class sec_clock {
 ...
 public:
  sec_clock() : t_(std::chrono::high_resolution_clock::now()) {}
