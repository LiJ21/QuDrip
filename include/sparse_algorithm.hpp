...
double getKrylovReprAd(const OP_MAT& H, const PSI_MAT& psi, VEC_MAT& v,
                       VEC_MAT& w, Matrix& KR) {
  ...
  while (n > eps) {
    ...
    std::cout << "Lanczos idx, err = " << idx << ", " << std::sqrt(n)
              << std::endl;
    ++idx;
...
//====================================================================================================
template <typename OP_MAT, typename VEC_MAT, typename PSI_MAT>
Matrix getKrylovReprAd(const OP_MAT& H, const PSI_MAT& psi, VEC_MAT& v,
                       VEC_MAT& w, size_t kry_dim, double& err) {
  v = psi / std::sqrt(norm(psi));
  std::vector<value_type> a, b;
  a.reserve(10);
  b.reserve(10);
...
std::vector<Matrix> getKrylovRepr(const OP_MAT& H, const PSI_MAT& psi,
                                  size_t KryDim) {
  std::vector<Matrix> v;
  v.reserve(10);
...
void getKrylovGS(const OP_MAT& H, const PSI_MAT& psi, const Matrix& GS,
                 VEC_MAT& v, VEC_MAT& w, RES_MAT&& psi_gs) {
  auto KryDim = GS.rows();
  v = psi / std::sqrt(norm(psi));
  w.setZero();
  auto alpha = 0.0;
...
template <typename OP_MAT, typename VEC_MAT, typename PSI_MAT>
Matrix getKrylovGS(const OP_MAT& H, const PSI_MAT& psi, const Matrix& GS,
                   VEC_MAT& v, VEC_MAT& w) {
  auto KryDim = GS.rows();
  v = psi / std::sqrt(norm(psi));
  w.setZero();
...
class LanczosSolver {
 ...
 public:
  ...
  template <typename OP_MAT>
  void getGroundState(const OP_MAT& H, State<index_type>& psi,
                      int tstp) noexcept {
    assert(tstp < psi.nt());
    auto Mat = Hermitian::getKrylovReprAd(H, psi(tstp).mat(), helper1, helper2,
                                          kry_dim_, err);
    ...
    psi(tstp).mat() = Hermitian::getKrylovGS(H, psi(tstp).mat(),
...
int expandBasis(const SpMatrix& H, Matrix& V, std::vector<value_type>& h,
...
void getState(Matrix& V, const OP_MAT& H, const EV_TYPE& evs, PSI_MAT&& psi,
              const Matrix& V0, int s_idx, herm_type<0> dummy) {
...
template <typename OP_MAT, typename PSI_MAT, typename EV_TYPE>
void getState(Matrix& V, const OP_MAT& H, const EV_TYPE& evs, PSI_MAT&& psi,
              const Matrix& V0, int s_idx, herm_type<1> dummy) {
  double n = 10.;
  int idx = 0;
...
class ArnoldiSolver {
 ...
 public:
  ...
  void updateKrylovRepr(const OP_MAT& H, int add_dim = -1) noexcept {
    if (add_dim == -1 || kry_dim_ < act_dim_ + add_dim)
      add_dim = kry_dim_ - act_dim_;
    std::vector<value_type> h;
    h.reserve(add_dim);
    double n = 10.0;
  ...
  template <typename OP_MAT>
  void evolve(const OP_MAT& H, State<index_type>& psi, int tstp, double h,
              int kry_dim_rt = 0) noexcept {
    assert(tstp < psi.nt() - 1);
  ...
  template <typename OP_MAT>
  void getGroundState(const OP_MAT& H, State<index_type>& psi, int tstp,
                      int add_dim = -1) noexcept {
    ...
    getState(V, H, eigensolver.eigenvectors(), psi(tstp).mat(), V0, 0,
             herm_type<herm>());
    err_ = std::sqrt(Hermitian::norm(
        (eigensolver.eigenvalues().block(
             0, 0, std::min((int)evs_.rows(), act_dim_), 1) -
