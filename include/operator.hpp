...
inline void Operator::full_branch(trans_index_type &trans_index,
  ...
  for (int i_op = ops_.size() - 1; i_op >= 0; --i_op) {
    ...
    for (; idx_b != idx_e; ++idx_b)  // convert total index to "label index"
    {
      ...
      for (auto it = idx_ww; it != idx_w;
           ++it)  // convert "label index" to the total index
      {
        if (*it != 0) {
          op->feed_idx(*it - 1);
          (*it) = trans_index + 1;
...
#ifdef PRINT_TREE
    std::cout << " " << std::bitset<6>((*(idx_b + j)) - 1);
    std::flush(std::cout);
#endif
  }
}
