...
class McManager {
 public:
  ...
  McManager(TS* p_psi, int* argc, char*** argv)
      : nt_(p_psi->nt()), p_psi_(p_psi), Nmc_(p_psi->nt(), 0) {
    MPI_Init(argc, argv);
  ...
  McManager(int nt, TS* p_psi, int* argc, char*** argv)
      : nt_(nt), p_psi_(p_psi), Nmc_(nt, 0) {
    MPI_Init(argc, argv);
    MPI_Comm_size(MPI_COMM_WORLD, &Nproc_);
    MPI_Comm_rank(MPI_COMM_WORLD, &my_rank_);
  }
  ~McManager() { MPI_Finalize(); }
#else

  McManager(int nt, TS* p_psi)
      : nt_(nt), p_psi_(p_psi), my_rank_(0), Nproc_(1), Nmc_(nt, 0) {}
  McManager(TS* p_psi)
      : nt_(p_psi->nt()),
        p_psi_(p_psi),
        my_rank_(0),
  ...
  void addAux(const std::string& name, const std::string& name2,
              const functor_type& func, int dim, int aux_type = MC_COMPLEX) {
  ...
  void collectAll() {
    ...
    for (auto& data : data_) {
      ...
      if (my_rank_ == 0) {
        MPI_Reduce(MPI_IN_PLACE, (data.second).data(), size, MPI_DOUBLE_COMPLEX,
                   MPI_SUM, 0, MPI_COMM_WORLD);
        data.second *= 1. / Nproc_;
      } else {
        MPI_Reduce((data.second).data(), (data.second).data(), size,
  ...
  void outFile(double h, const std::string& fname,
               const std::vector<std::string>& obs_names, int precision = 6) {
    ...
    for (int t = 0; t < nt_; ++t) {
      for (int idx = 0; idx < dim; ++idx) {
        if (dim > 1)
          label = std::to_string(t * h) + "\t" + std::to_string(idx);
        else
          label = std::to_string(t * h);

        for (auto& n : obs_names) {
          auto it = data_.find(n);
          label += "\t" + std::to_string((it->second)(t, idx).real());
          if (type_.find(n)->second == MC_COMPLEX)
            label += "\t" + std::to_string((it->second)(t, idx).imag());
        }
        f << label << std::endl;
      }
      if (dim > 1) f << std::endl;
