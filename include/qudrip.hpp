...
inline constexpr short BIT_LIMIT = 64;
inline const value_type II = value_type(0.0, 1.0);
//--------------------------------------------------------------------
// range functions
inline range_type range(const size_t N) {
  range_type r(N);
  std::iota(r.begin(), r.end(), 0);
  return r;
}

inline range_type range(const size_t M, const size_t N) {
  range_type r(N - M);
  std::iota(r.begin(), r.end(), M);
  return r;
}

inline range_type range(const size_t M, const size_t N, const size_t STEP) {
