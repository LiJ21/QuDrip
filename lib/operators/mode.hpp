...
class modeOp : public elOpBase
{
	...
	public:
		modeOp(mode_type & index) : 
			...
			index_(index), ndim_(index.range())
		{
		}

		Operator operator()(size_t site)
		{
			return Operator(std::static_pointer_cast<elOpBase>(std::make_shared<modeOp>(*this)));
		}
...
//==========================================================================================
template<typename T>
void modeOp<T>::branch(idx_it_type & idx_b, val_it_type & val_b, idx_it_type & idx_w, val_it_type & val_w)
{
