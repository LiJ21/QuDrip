...
class hopBitOp : public elOpBase
{
	...
	public:
		...
		Operator operator()(size_t site2, size_t site1)
		{
			site1_ = site1;
			site2_ = site2;
			return Operator(std::static_pointer_cast<elOpBase>(std::make_shared<hopBitOp>(*this)));
		}
...
//==========================================================================
template <typename T>
void hopBitOp<T>::branch(idx_it_type & idx_b, val_it_type & val_b, idx_it_type & idx_w, val_it_type & val_w)
{
	int i = 0;
	value_type val0;
	idxv_type idx0;
	if(*idx_b != 0)
	{
		...
		{
			//std::cout << " Hop bit: " << site1_ << "," << site2_ << " for " << std::bitset<6>(*idx_b - 1) << " yield counts ";
			//std::cout << bits[site1_] << "," << bits[site2_] << std::endl;
			auto jw_set1 = bits_type(bit_convert<idxv_type>(bits) >> site1_ + 1).count();
			bits[site1_] = 0;

			auto jw_set2 = bits_type(bit_convert<idxv_type>(bits) >> site2_ + 1).count();
			bits[site2_] = 1;
